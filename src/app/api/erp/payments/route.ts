...
import { createDemoPayment } from "@/lib/erp/demo-store";
import { createPaymentSchema } from "@/lib/erp/schemas";
...
export async function POST(request: Request) {
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: createDemoPayment(parsed.data) }, 201), context);
    } catch (error) {
      return withDemoHeader(
        json({ error: error instanceof Error ? error.message : "Payment gagal dibuat." }, 422),
        context,
      );
    }
  }

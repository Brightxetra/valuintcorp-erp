...
import { summarizeDemoRawImportBatch } from "@/lib/erp/demo-store";
...
export async function POST(request: Request) {
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: summarizeDemoRawImportBatch(parsed.data) }), context);
    } catch (error) {
      return withDemoHeader(json({ error: error instanceof Error ? error.message : "Ringkasan import gagal." }, 422), context);
    }
  }

  const supabase = createRequestSupabaseClient(request);
  const { error } = await supabase.rpc("summarize_raw_import_batch", {

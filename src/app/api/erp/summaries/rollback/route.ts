...
import { rollbackDemoDailySummary } from "@/lib/erp/demo-store";
...
export async function POST(request: Request) {
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: rollbackDemoDailySummary(parsed.data) }), context);
    } catch (error) {
      return withDemoHeader(json({ error: error instanceof Error ? error.message : "Rollback ringkasan gagal." }, 422), context);
    }
  }

  const supabase = createRequestSupabaseClient(request);
  const { error } = await supabase.rpc("rollback_daily_summary", {

...
import { reverseDemoFixedAssetDocument } from "@/lib/erp/demo-store";
import { reverseFixedAssetDocumentSchema } from "@/lib/erp/schemas";
...
export async function POST(request: Request) {
  ...
  const parsed = reverseFixedAssetDocumentSchema.safeParse(payload);
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: reverseDemoFixedAssetDocument(parsed.data) }), context);
    } catch (error) {
      return withDemoHeader(
        json({ error: error instanceof Error ? error.message : "Reversal aset gagal dipost." }, 422),
        context,
      );
    }
  }
  ...
  const { error } = await supabase.rpc("reverse_fixed_asset_document", {

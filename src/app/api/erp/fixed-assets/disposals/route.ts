...
import { createDemoFixedAssetDisposal } from "@/lib/erp/demo-store";
import { fixedAssetDisposalSchema } from "@/lib/erp/schemas";
...
export async function POST(request: Request) {
  ...
  const parsed = fixedAssetDisposalSchema.safeParse(payload);
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: createDemoFixedAssetDisposal(parsed.data) }, 201), context);
    } catch (error) {
      return withDemoHeader(
        json({ error: error instanceof Error ? error.message : "Pelepasan aset gagal dipost." }, 422),
        context,
      );
    }
  }
  ...
  const { error } = await supabase.rpc("post_fixed_asset_disposal", {

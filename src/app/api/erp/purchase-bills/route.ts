...
import { createDemoPurchaseBill } from "@/lib/erp/demo-store";
import { createPurchaseBillSchema } from "@/lib/erp/schemas";
...
export async function POST(request: Request) {
  ...
  const parsed = createPurchaseBillSchema.safeParse(payload);
  ...
  if (context.demoMode) {
    try {
      return withDemoHeader(json({ workspace: createDemoPurchaseBill(parsed.data) }, 201), context);
    } catch (error) {
      return withDemoHeader(
        json({ error: error instanceof Error ? error.message : "Bill gagal dibuat." }, 422),
        context,
      );
    }
  }
  ...
  const { error } = await supabase.rpc("post_purchase_bill", {
